...
Generated by TabGenius"""
    ...
    with tab2:# Music Lessons tab (this is your new tab)
        with tab2:  # Music Lessons tab
    st.header("🎓 Music Lessons")
    st.write("Welcome to TabGenius Music Academy!")
    
    # Beginner lessons
    st.subheader("🟢 Beginner Lessons")
    with st.expander("📖 Basic Chords (15 mins)"):
        st.write("Learn essential G, C, D, Em chords")
        st.code("""G Major Chord:
E|---3---|
B|---0---|
G|---0---|
D|---0---|
...
E|---3---|""", language="text")
        ...
        if st.button("✅ Mark Complete", key="basic_chords"):
            st.success("Lesson completed! 🎉")
    
    with st.expander("📖 Strumming Patterns (20 mins)"):
        st.write("Master down-up strumming basics")
        st.code("""Basic Pattern: D-D-U-U-D-U
Rhythm: 1 e + a 2 e + a
        D   D U U D U""", language="text")
        
        if st.button("✅ Mark Complete", key="strumming"):
            st.success("Great job! 🎸")
    
    # Premium lessons preview
    if st.session_state.user_info['tier'] == 'free':
        st.subheader("🔒 Premium Lessons")
        st.warning("Upgrade to access intermediate and advanced lessons!")
        st.button("Upgrade Now", key="upgrade_lessons")
    else:
        st.subheader("🟡 Intermediate Lessons")
        st.info("Advanced lessons available for premium members!")
